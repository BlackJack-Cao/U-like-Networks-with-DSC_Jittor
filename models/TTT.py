...
class RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = hidden_states.astype(jt.float32)
...
class RotaryEmbedding(nn.Module):
    def __init__(
        ...
        self.base = base
        inv_freq = 1.0 / (self.base ** (jt.arange(0, self.dim, 2).astype(jt.float32) / self.dim))
        ...
        if causal_conv1d_fn is None:
            if cache_params is not None:
                if cache_params.seqlen_offset > 0:
                    ...
                    conv_state = jt.roll(conv_state, shifts=-1, dims=-1)
                    conv_state[:, :, -1] = hidden_states[:, :, 0]
                    cache_params.conv_states_dic["pre_conv"][self.layer_idx].copy_(conv_state)
                    hidden_states = jt.sum(conv_state * self.conv.weight[:, 0, :], dim=-1)
...
def scan(f, init, xs, out, checkpoint_group=0):
    ...
    if checkpoint_group > 0:
        ...
        for k in range(0, num_items, ckpt_every_n):
            # Jittor doesn't have checkpoint, so we'll skip it for now
            # carry = torch.utils.checkpoint.checkpoint(
            #     scan_fn, carry, k, min(k + ckpt_every_n, num_items), use_reentrant=False
            # )
            carry = scan_fn(carry, k, min(k + ckpt_every_n, num_items))
    else:
...
def gelu_bwd(x):
    tanh_out = jt.tanh(0.79788456 * x * (1 + 0.044715 * x * x))
